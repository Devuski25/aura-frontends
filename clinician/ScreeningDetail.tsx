...
function InfoCard({ title, className, children }: { title: string; className?: string; children: React.ReactNode }) {
  return (
    <div className={cn("rounded-[14px] border border-aura-line/60 bg-aura-sage/40 py-5 px-6", className)}>
...
function ResultBanner({ screening }: { screening: ScreeningDetail }) {
  ...
  return (
    <div role="status" className={cn("mb-6 flex items-center gap-4 rounded-2xl border p-5 shadow-sm", cfg.container)}>
      <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-white shadow-sm">{cfg.icon}</div>
      <div>
...
export function ScreeningDetail() {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl pb-10">
      ...
      {/* Screening Details */}
      <InfoCard title="Screening Details" className="mt-6">
      ...
      {/* Review Status */}
      <div className="mt-6 rounded-[14px] border border-aura-line bg-white py-5 px-6">
        <h2 className="mb-4 flex items-center gap-2 font-display text-[13px] font-semibold uppercase tracking-[0.04em] text-aura-muted">
          Review Status
        </h2>
